import os
from google.cloud import dataplex_v1
from google.cloud.dataplex_v1 import DataTaxonomyServiceClient, DataScanServiceClient
from google.protobuf import struct_pb2

# Configuration
PROJECT_ID = os.environ.get("GOOGLE_CLOUD_PROJECT")
LOCATION = "global" # For Aspect Types
BQ_LOCATION = "eu" # For BigQuery entries
DATASET_ID = "retail_synthetic_data"


def create_aspect_type():
    client = dataplex_v1.CatalogServiceClient()
    parent = f"projects/{PROJECT_ID}/locations/{LOCATION}"
    aspect_type_id = "data-governance-aspect"
    
    aspect_type = dataplex_v1.AspectType()
    aspect_type.description = "Governance metadata for Retail data"
    aspect_type.metadata_template = dataplex_v1.AspectType.MetadataTemplate()
    aspect_type.metadata_template.name = "governance_info"
    aspect_type.metadata_template.type = "RECORD"
    
    # Add fields to aspect type
    field_owner = dataplex_v1.AspectType.MetadataTemplate()
    field_owner.name = "owner"
    field_owner.type = "STRING"
    field_owner.constraints.required = True
    field_owner.index = 1 # Added index
    
    field_pii = dataplex_v1.AspectType.MetadataTemplate()
    field_pii.name = "contains_pii"
    field_pii.type = "BOOL"
    field_pii.index = 2 # Added index
    
    field_layer = dataplex_v1.AspectType.MetadataTemplate()
    field_layer.name = "layer"
    field_layer.type = "STRING" # Values: raw, datalake
    field_layer.index = 3 # Added index
    
    field_env = dataplex_v1.AspectType.MetadataTemplate()
    field_env.name = "environment"
    field_env.type = "STRING" # Values: UAT, Prod
    field_env.index = 4 # Added index
    
    aspect_type.metadata_template.record_fields.extend([field_owner, field_pii, field_layer, field_env])
    
    try:
        operation = client.create_aspect_type(
            parent=parent,
            aspect_type_id=aspect_type_id,
            aspect_type=aspect_type
        )
        print(f"Creating aspect type {aspect_type_id}...")
        operation.result()
        print("Aspect type created.")
    except Exception as e:
        print(f"Aspect type might already exist: {e}")
    return f"projects/{PROJECT_ID}/locations/{LOCATION}/aspectTypes/{aspect_type_id}"

# def create_insights_aspect_type():
#     client = dataplex_v1.CatalogServiceClient()
#     parent = f"projects/{PROJECT_ID}/locations/{LOCATION}"
#     aspect_type_id = "data-insights-aspect"
#     
#     aspect_type = dataplex_v1.AspectType()
#     aspect_type.description = "Insights and Golden Queries for Retail data"
#     aspect_type.metadata_template = dataplex_v1.AspectType.MetadataTemplate()
#     aspect_type.metadata_template.name = "insights_info"
#     aspect_type.metadata_template.type = "RECORD"
#     
#     field_desc = dataplex_v1.AspectType.MetadataTemplate()
#     field_desc.name = "description"
#     field_desc.type = "STRING"
#     field_desc.index = 1
#     
#     field_golden_query = dataplex_v1.AspectType.MetadataTemplate()
#     field_golden_query.name = "golden_query"
#     field_golden_query.type = "STRING"
#     field_golden_query.index = 2
#     
#     aspect_type.metadata_template.record_fields.extend([field_desc, field_golden_query])
#     
#     try:
#         operation = client.create_aspect_type(
#             parent=parent,
#             aspect_type_id=aspect_type_id,
#             aspect_type=aspect_type
#         )
#         print(f"Creating aspect type {aspect_type_id}...")
#         operation.result()
#         print("Aspect type created.")
#     except Exception as e:
#         print(f"Aspect type might already exist: {e}")
#     return f"projects/{PROJECT_ID}/locations/{LOCATION}/aspectTypes/{aspect_type_id}"

def get_entry_name_from_bq_table(project_id, dataset_id, table_id):
    # Directly construct the entry name as search is unreliable in this environment
    # and the fallback format is correct.
    entry_id = f"bigquery.googleapis.com/projects/{project_id}/datasets/{dataset_id}/tables/{table_id}"
    entry_name = f"projects/{project_id}/locations/{BQ_LOCATION}/entryGroups/@bigquery/entries/{entry_id}"
    return entry_name

def tag_table(table_name, aspect_type_name, owner, contains_pii, layer, environment="Prod"):
    client = dataplex_v1.CatalogServiceClient()
    
    entry_name = get_entry_name_from_bq_table(PROJECT_ID, DATASET_ID, table_name)
    if not entry_name:
        print(f"Could not find harvested entry for {table_name}, skipping.")
        return
    
    aspect_data = struct_pb2.Struct()
    aspect_data.update({
        "owner": owner,
        "contains_pii": contains_pii,
        "layer": layer,
        "environment": environment
    })
    
    entry = dataplex_v1.Entry()
    entry.name = entry_name
    # The key for aspects map must be in the format project.location.aspectType, not the full resource name
    # aspect_type_name is projects/{project}/locations/{location}/aspectTypes/{aspectType}
    parts = aspect_type_name.split('/')
    if len(parts) >= 6:
        project = parts[1]
        location = parts[3]
        aspect_type_id = parts[5]
        aspect_key = f"{project}.{location}.{aspect_type_id}"
    else:
        aspect_key = aspect_type_name # Fallback, though unlikely to work if format is wrong
    
    entry.aspects = {
        aspect_key: dataplex_v1.Aspect(
            aspect_type=aspect_type_name,
            data=aspect_data
        )
    }
    
    try:
        client.update_entry(entry=entry, update_mask={"paths": [f"aspects"]})
        print(f"Updated harvested entry and tagged {table_name}")
    except Exception as e:
        print(f"Failed to update entry for {table_name}: {e}")

# def tag_insights(table_name, aspect_type_name, description, golden_query):
#     client = dataplex_v1.CatalogServiceClient()
#     entry_id = f"bq-{DATASET_ID}-{table_name}"
#     parent = f"projects/{PROJECT_ID}/locations/{LOCATION}/entryGroups/{ENTRY_GROUP_ID}"
#     name = f"{parent}/entries/{entry_id}"
#     
#     aspect_data = struct_pb2.Struct()
#     aspect_data.update({
#         "description": description,
#         "golden_query": golden_query
#     })
#     
#     entry = dataplex_v1.Entry()
#     entry.name = name
#     entry.aspects = {
#         aspect_type_name: dataplex_v1.Aspect(
#             aspect_type=aspect_type_name,
#             data=aspect_data
#         )
#     }
#     
#     try:
#         # We assume the entry exists from tag_table, so we update.
#         # To avoid overwriting other aspects, we should ideally use a specific field mask,
#         # but for simplicity we are using the aspects map. 
#         # Note: This might overwrite other aspects if not careful in production.
#         client.update_entry(entry=entry, update_mask={"paths": [f"aspects"]})
#         print(f"Tagged {table_name} with insights")
#     except Exception as e:
#         print(f"Failed to tag insights for {table_name}: {e}")

# def create_dq_scan(table_name):
#     client = dataplex_v1.DataScanServiceClient()
#     parent = f"projects/{PROJECT_ID}/locations/{LOCATION}"
#     scan_id = f"dq-scan-{table_name}"
#     
#     # Define rules based on table
#     rules = []
#     if table_name == "customers":
#         rules = [
#             dataplex_v1.DataQualityRule(column="email", non_null_expectation={}),
#             dataplex_v1.DataQualityRule(column="phone", regex_expectation={"regex": r"^\+?[0-9\s\-()]+$"})
#         ]
#     elif table_name == "products":
#         rules = [
#             dataplex_v1.DataQualityRule(column="price", non_null_expectation={}),
#             dataplex_v1.DataQualityRule(column="price", range_expectation={"min_value": "0", "max_value": "1000"}) # Use strings for min/max
#         ]
#     
#     if not rules:
#         return
#     
#     data_quality_spec = dataplex_v1.DataQualitySpec(rules=rules)
#     
#     data_scan = dataplex_v1.DataScan()
#     data_scan.data.entity = f"projects/{PROJECT_ID}/datasets/{DATASET_ID}/tables/{table_name}"
#     data_scan.data_quality_spec = data_quality_spec
#     # data_scan.result_pubsub_topic = f"projects/{PROJECT_ID}/topics/dq-results" # Optional
#     
#     try:
#         operation = client.create_data_scan(
#             parent=parent,
#             data_scan=data_scan,
#             data_scan_id=scan_id
#         )
#         print(f"Creating DQ scan for {table_name}...")
#         operation.result()
#         print(f"DQ scan created for {table_name}")
#     except Exception as e:
#         print(f"DQ scan might already exist for {table_name}: {e}")

# def create_insights_scan(table_name):
#     client = dataplex_v1.DataScanServiceClient()
#     parent = f"projects/{PROJECT_ID}/locations/{LOCATION}"
#     scan_id = f"insights-scan-{table_name}"
#     
#     data_scan = dataplex_v1.DataScan()
#     data_scan.data.resource = f"//bigquery.googleapis.com/projects/{PROJECT_ID}/datasets/{DATASET_ID}/tables/{table_name}"
#     data_scan.execution_spec.trigger.on_demand = {}
#     data_scan.type_ = "DATA_DOCUMENTATION"
#     data_scan.data_documentation_spec = {}
#     
#     try:
#         operation = client.create_data_scan(
#             parent=parent,
#             data_scan=data_scan,
#             data_scan_id=scan_id
#         )
#         print(f"Creating Insights scan for {table_name}...")
#         operation.result()
#         print(f"Insights scan created for {table_name}")
#     except Exception as e:
#         print(f"Insights scan might already exist for {table_name}: {e}")

if __name__ == "__main__":
    if not PROJECT_ID:
        print("Please set GOOGLE_CLOUD_PROJECT environment variable.")
        exit(1)
    
    aspect_type_name = create_aspect_type()
    
    tag_table("customers", aspect_type_name, "Data Privacy Team", True, "raw")
    tag_table("products", aspect_type_name, "Merchandising Team", False, "raw")
    tag_table("orders", aspect_type_name, "Sales Team", False, "datalake")
    tag_table("transactions", aspect_type_name, "Finance Team", False, "datalake")
    
    # insights_aspect_type_name = create_insights_aspect_type()
    # tag_insights("customers", insights_aspect_type_name, "Table containing customer profile and contact information.", "SELECT * FROM customers WHERE country = 'UK'")
    # tag_insights("products", insights_aspect_type_name, "Product catalog with pricing and categories.", "SELECT * FROM products WHERE price > 100")
    
    # create_dq_scan("customers")
    # create_dq_scan("products")
    
    # create_insights_scan("customers")
    # create_insights_scan("products")
